...
## 阶段 3 🟡 清理死代码与噪声（可维护性） ✅ 已完成 2026-08-02
**风险：低。删未用导入/字段，纯瘦身。**
...
## 阶段 4 🟢 质量与健壮性增强（按需，非阻塞） ✅ favicon 兜底已具备；其余可选
**风险：中低。涉及少量逻辑改动，建议单独立 PR。**

- [x] **favicon 兜底**：`LinkBlock` 的图标 `onError` 时回退到站点首字母/占位图，避免离线时全空。（经核查此逻辑在 `LinkBlock.tsx` 已存在：`imageUrl` 加载失败 → `imageError` → 显示标题首字母，无需新增。）

...
> 当前版本 **2.0.0** — 本地优先、隐私优先的书签 / 新标签页导航面板。

一个本地优先、隐私优先的书签 / 新标签页导航面板。单页应用，数据默认存浏览器 `localStorage`，可选通过 WebDAV 备份/恢复。

## 隐私与定位（红线）

本项目从架构上承诺 **本地优先、数据归用户自己**：

- **完全免费**：没有订阅、内购、广告或诱导付费。
- **没有后端服务器**：不运营任何云端服务，书签不会经过我们的服务器。
- **数据 100% 存本机**：书签与配置只保存在浏览器 `localStorage`，断网也能用。
- **同步只走你自己的 WebDAV**：跨设备漫游由你自己的网盘（坚果云 / Nextcloud 等）完成，密钥与数据都在你手里；可选端到端加密。
- **不埋点、不追踪、不加账号**：没有任何分析统计上报，也无需注册登录——打开即用。
- **不接第三方分析 SDK**（如 Google Analytics）：一旦上报书签就违背上述承诺。

> 因此本项目**不会**添加：账户/登录、云端备份、付费墙、广告、遥测上报。这些"不做"本身就是差异化卖点。
